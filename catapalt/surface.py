...
class SurfaceAnalyzer:
    ...
    def find_nodes_per_area(self, thresh):
        surface_atoms = self.slab[[idx for idx,tag in enumerate(self.slab.get_tags()) if tag==1]]
        surface_atoms = surface_atoms.repeat((3,3,1))
        
        cell_vectors = AseAtomsAdaptor.get_structure(self.slab).lattice.matrix[:, 0:2]
        cell_vectors = [vector for vector in cell_vectors if not np.allclose([0,0], vector)]
        
        delaunay_2d_mesh = Delaunay(surface_atoms.get_positions()[:,0:2])
        new_positions, new_faces = simplify_mesh(np.array(surface_atoms.get_positions(), np.double), np.array(delaunay_2d_mesh.simplices, np.uint32), 3, max_err = thresh)
        
        planes_3x = [Plane(new_positions[new_face[0]], new_positions[new_face[1]], new_positions[new_face[2]], idx) for idx, new_face in enumerate(new_faces)]
        ...
        return (len(planes_3x)/envelope_area) # per area doesnt make sense because some features are infinite. What can I do instead?

    def _drop_edge_planes(self, planes, cell_vectors):
        planes_to_keep = []
        inner_slab = Polygon([cell_vectors[0]+cell_vectors[1], 2*cell_vectors[0]+cell_vectors[1],
                             cell_vectors[0]+2*cell_vectors[1], 2*cell_vectors[0]+2*cell_vectors[1]])
        for plane in planes:
    ...
    def _drop_duplicate_planes(self, planes):
        ...
        for plane in planes:
            idx = plane.identity
            planes_to_compare = [plane_now for plane_now in planes_reduced if (plane_now.identity != idx)]
            norms = [plane_now.norm for plane_now in planes_to_compare]
            norm_mag = np.sqrt(plane.norm.dot(plane.norm))
            norms_cos = [round(np.dot(plane.norm, norm_now)/(np.sqrt(norm_now.dot(norm_now))*norm_mag),3) for norm_now in norms]
            norms_same = [bool_index for bool_index, cos in enumerate(norms_cos) if (np.abs(cos) >= 0.996)]
            for true_index in norms_same:
                if planes_to_compare[true_index].point_is_on(plane.points[0]):
                    idx_to_delete = [idx_on for idx_on, val in enumerate(planes_reduced) if (val.identity == plane.identity)][0]
